...
def get_trimmed_reads(wildcards, units):
    if units.loc[wildcards.unit, ["fq2"]].isna().all():
        ...
        return (
            rules.post_rename_fastq_pe.output.r1,
            rules.post_rename_fastq_pe.output.r2,
        )
...
def threads_calculator(read_type="pe"):
    if read_type == "se":
        if conservative_cpu_count(reserve_cores=2, max_cores=99) > 2:
            return conservative_cpu_count(reserve_cores=2, max_cores=99) / 2
        else:
            return 1
    else:
        if conservative_cpu_count(reserve_cores=2, max_cores=99) > 4:
            return conservative_cpu_count(reserve_cores=2, max_cores=99) / 4
        else:
            return 1
...
def get_units_by_sample(
    wildcards,
    samples,
    label="units",
    prefix=resolve_results_filepath(
        config.get("paths").get("results_dir"), "reads/sorted/"
    ),
    suffix="_sorted.cram",
):
    return [
        prefix + i + suffix
        for i in samples.loc[wildcards.sample, [label]][0].split(",")
    ]
