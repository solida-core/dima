...
rule pre_rename_fastq_pe:
    ...
    output:
        r1=resolve_results_filepath(
            config.get("paths").get("results_dir"), "reads/untrimmed/{unit}-R1.fq.gz"
        ),
        r2=resolve_results_filepath(
            config.get("paths").get("results_dir"), "reads/untrimmed/{unit}-R2.fq.gz"
        ),
    log:
        resolve_results_filepath(
            config.get("paths").get("results_dir"), "logs/bash/pe/{unit}_cp.log"
        ),
    conda:
...
rule trim_galore_pe:
    ...
    resources:
        tmpdir=config.get("paths").get("tmp_dir"),
    threads: threads_calculator(read_type="pe")
    shell:
...
rule post_rename_fastq_pe:
    ...
    log:
        resolve_results_filepath(
            config.get("paths").get("results_dir"), "logs/bash/pe/{unit}_mv.log"
        ),
    conda:
        resolve_single_filepath(
            config.get("paths").get("workdir"), "workflow/envs/bash.yaml"
