...
rule gatk_BQSR_data_processing:
    ...
    params:
        ...
        known_sites=get_known_sites(
            config.get("params").get("gatk_BQSR").get("known_sites")
        ),
    log:
        resolve_results_filepath(
            config.get("paths").get("results_dir"),
...
rule gatk_ApplyBQSR:
    ...
    output:
        ...
        bai=protected(
            resolve_results_filepath(
                config.get("paths").get("results_dir"),
                "reads/recalibrated/{sample}.dedup.recal.bai",
            )
        ),
    conda:
        resolve_single_filepath(
...
rule gatk_BQSR_quality_control:
    input:
        ...
        pre=resolve_results_filepath(
            config.get("paths").get("results_dir"),
            "reads/recalibrated/{sample}.recalibrate.grp",
        ),
    output:
        post=resolve_results_filepath(
            config.get("paths").get("results_dir"),
            "reads/recalibrated/{sample}.post.recalibrate.grp",
        ),
    ...
    log:
        b=resolve_results_filepath(
            config.get("paths").get("results_dir"),
            "logs/gatk/BaseRecalibrator/{sample}_BQSR_quality_control_info.log",
        ),
        a=resolve_results_filepath(
            config.get("paths").get("results_dir"),
            "logs/gatk/AnalyzeCovariates/{sample}_BQSR_quality_control_cov_info.log",
        ),
    benchmark:
        resolve_results_filepath(
            config.get("paths").get("results_dir"),
